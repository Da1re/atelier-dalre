...
const RAW_PROJECTS: Project[] = [
  {
    ...
    title: "국립과천과학관 포탈시스템 재구축 및 클라우드 전환",
    ...
    highlights: [
      ...
      {
        title: "에디터 콘텐츠 인터랙션 — 선언적 액션 props 계약으로 보안·자유도 양립",
        description:
          "콘텐츠 작성 자유도와 보안이 상충하는 요구가 있었음. 직접 우회 대신, 위험한 동적 실행은 차단하고 navigate·모달 호출 같은 인터랙션은 선언적 props 계약(contract)으로 콘텐츠에 기술하도록 모델링. 뷰어가 그 계약을 React 핸들러로 해석해 동작을 수행하는 구조로 설계해, 보안과 자유도를 동시에 만족시킴.",
      },
      {
        title: "동적 메뉴 시스템 — 사용자 정의 메뉴 기반 라우터 자동 생성",
        description:
          "관리자가 콘텐츠·카드·게시판 등 사전 정의된 템플릿 타입으로 메뉴를 직접 생성하면, 서버가 내려준 메뉴 트리를 클라이언트에서 라우터 객체로 변환해 페이지가 동적으로 구성되는 구조 설계. 메뉴 고유 식별자(uniqueValue) 기반으로 페이지 컴포넌트를 매핑하고 templateType별 공통 Wrapper를 자동 할당해, 메뉴 추가 시 별도 코드 수정 없이 라우팅이 확장되도록 구현. @dnd-kit 기반 계층 트리 관리(optimistic update 적용)와 서버 응답 기반 권한 모델로 메뉴·접근 제어를 일원화.",
      },
      {
...
콘텐츠 작성 자유도와 보안을 양립시켜야 했던 요구에서는, 위험한 동적 실행은 차단하되 navigate·모달 같은 동작은 선언적 props 계약으로 분리해 뷰어가 해석하도록 설계했습니다. 보안과 자유도가 충돌할 때 직접 우회보다 데이터 계약으로 간극을 메우는 접근의 가치를 체감했습니다.
...
공통 디자인 시스템을 처음부터 설계하면서 "단순히 예쁜 컴포넌트"가 아닌, 비즈니스 로직을 캡슐화한 재사용 가능한 컴포넌트의 가치를 체감했습니다.`,
  ...
  },
  {
    ...
    title: "AI 어시스턴트 채팅 서비스",
    subtitle: "SSE 기반 스트리밍 채팅 시스템",
    company: "(주)세이코어",
  ...
  },
  {
    ...
    highlights: [
      ...
      {
        title: "다중 첨부 공유 — 외부 솔루션 제약을 도메인 재설계로 우회",
        description:
          "외부 화상회의 솔루션의 공유 방식 제약으로 다중 참가자 동시 공유가 어려웠음. 백엔드 업로드 완료 이벤트를 실시간으로 수신해 첨부 목록을 동기화하고, 참가자 전원에게 공유가 전파되도록 흐름을 재설계해 제약을 우회. 외부 솔루션의 한계 안에서 도메인을 다시 설계한 사례.",
      },
      {
        title:
          "전역 Auth Layer 설계 — 다중 회원 타입 라우팅 & 토큰·userInfo 중앙 관리",
        description:
          "역할별로 접근 가능한 영역이 다른 다중 회원 타입 시스템에서, Jotai 기반 전역 인증 상태 관리 아키텍처를 설계. Role 기반 route guard를 전역 레이어로 끌어올려 각 페이지에서 인증 분기 로직을 제거하고, Access Token Silent Refresh와 만료 감지·로그인 리다이렉트를 하나의 흐름으로 묶음. userInfo set/get 공통 hook을 분리해 어느 페이지에서든 동일한 인터페이스로 사용자 정보에 접근하도록 정리.",
      },
      {
        title:
          "전역 개인정보 보호 컨트롤 — Layout 단일 진입점 & 페이지별 노출 분기",
        description:
          "개인정보 보호(마스킹) 제어를 어느 페이지에서든 단일 진입점으로 다룰 수 있도록 전역 레이아웃에 배치하는 도메인 흐름 설계. 페이지별 노출 플래그로 불필요한 화면에서는 컨트롤을 숨겨 일관된 UX 유지.",
      },
    ],
...
외부 솔루션 자체의 제약 — 공유 방식이 제한적인 구조 — 도 백엔드를 거쳐 참가자 전원에게 전파되도록 흐름을 다시 설계해 풀어냈고, 단순 솔루션 사용자가 아니라 "솔루션의 제약 안에서 도메인을 어떻게 다시 설계할지"까지 책임지는 경험을 했습니다.

역할마다 접근 가능한 영역이 갈리는 다중 회원 타입 시스템에서 전역 Auth Layer를 설계하면서, "인증·역할 분기는 페이지가 아니라 전역 관심사"라는 개념을 코드로 구현해보는 경험을 했습니다. 각 페이지마다 흩어져 있던 인증 체크와 role 분기를 단일 레이어로 응집시키면서 코드의 응집도와 유지보수성이 크게 향상됐고, userInfo 같은 공용 상태도 set/get hook으로 정리하면서 "전역 상태에 어떻게 접근하게 할 것인가"라는 인터페이스 설계의 감을 잡았습니다.

전역 개인정보 보호 컨트롤처럼 전역 진입점이 필요한 도메인을 설계해보면서, "어디서나 호출 가능한 UI"가 단순 모달이 아니라 어느 페이지에서 노출할지·숨길지까지 포함하는 도메인 흐름이라는 걸 체감했습니다.`,
    retroKeywords: [
      "프론트 주도 도메인 설계",
      "솔루션 제약 우회 · 도메인 재설계",
      "다중 회원 타입 Auth Layer",
